...
export default function UpdateProfileCompleteForm({ className = '', userTechs, allTechs }: Props) {
    ...
    const [expandedGroups, setExpandedGroups] = useState(() => {
        return new Set(
            techGroups
                .filter((group) => group.techs.some((tech) => userTechs.some((userTech) => userTech.id === tech.id)))
                .map((group) => group.key),
        )
    })
    ...
    const handleTechToggle = (techId: number, checked: boolean) => {
        if (checked) {
            ...
            setData(
                'techs',
                data.techs.filter((t) => t.id !== techId),
            )
        }
    }
        ...
        <section className={className}>
            <form onSubmit={submit} className="space-y-6">
                ...
                <div>
                    ...
                    {data.techs.length > 0 && (
                        <div className="mb-4 p-3 bg-muted/50 rounded-lg border border-border">
                            <p className="text-sm text-muted-foreground mb-2">
                                {data.techs.length} tecnología{data.techs.length !== 1 ? 's' : ''}{' '}
                                seleccionada{data.techs.length !== 1 ? 's' : ''}
                            </p>
                    ...
                    {/* Techs grouped by category */}
                    <div className="space-y-2">
                        {techGroups.map((group) => {
                            const selectedCount = group.techs.filter((t) =>
                                ...
                                >
                                    ...
                                    <div className="divide-y divide-border">
                                        {group.techs.map((tech) => {
                                            const userTech = data.techs.find(
                                                (t) => t.id === tech.id,
                                            )
                                                            ...
                                                            <div>
                                                                ...
                                                                >
                                                                    <input
                                                                        ...
                                                                        min="1"
                                                                        max="5"
                                                                        value={getTechProficiencySliderValue(
                                                                            userTech.proficiency,
                                                                        )}
                                                                        onChange={(e) =>
                                                                            handleTechUpdate(
