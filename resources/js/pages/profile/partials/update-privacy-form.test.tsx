...
describe('UpdatePrivacyForm', () => {
    it('renders the current phone and privacy state with helper copy', () => {
        ...
        expect(
            screen.getByRole('checkbox', { name: /Ser descubrible en el directorio/i }),
        ).toBeChecked()
        expect(
            screen.getByRole('checkbox', { name: /Mostrar actividad pública/i }),
        ).not.toBeChecked()

        expect(
            screen.getByText(/Cualquiera que visite tu perfil podrá ver tu correo/i),
        ).toBeInTheDocument()
        ...
        expect(
            screen.queryByText(/recibir emails opcionales de colaboración/i),
    ...
    it('shows inline errors and success feedback', () => {
        ...
        expect(screen.getAllByRole('alert')[1]).toHaveTextContent(
            'No se pudo guardar la visibilidad del teléfono',
        )
        expect(screen.getByRole('checkbox', { name: /Mostrar teléfono/i })).toHaveAttribute(
            'aria-describedby',
            'privacy-show_phone-error',
        )
