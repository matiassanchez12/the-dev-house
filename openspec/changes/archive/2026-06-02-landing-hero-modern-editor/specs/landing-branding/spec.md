...
The landing page hero MUST feature an animated background with staggered content reveal animations, and MUST compose a monospace wordmark signature alongside a static tech grid rather than a typing terminal.
...
- WHEN the page loads
- THEN the headline, subheadline, wordmark, tech grid, and CTA MUST fade in sequentially with staggered delays
- AND the wordmark and tech grid MUST be static once revealed (no continuous motion)
- AND no terminal or typing artefact SHALL be present in the hero source
